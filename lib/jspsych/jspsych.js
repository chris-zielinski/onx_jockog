...
* Initial jspsych.js is version 5.0.3 (2015)
* CREx-BLRI-AMU
...
jsPsych.data = (function() {
  ...
  module.write = function(data_object) {
	...
	// CREx AppReg - remove in default_data : 
	// 'icur': progress.current_trial_global, and 'iG': jsPsych.currentTimelineNodeID(),
	...
	var ext_data_object = $.extend({}, default_data, dataProperties, data_object, trial.data);
...
/** CREx add function to delete all Data before a new experiment*/
  module.deleteAllData = function(){
    allData = [];
  }
...
/*!! CREx Modig : trial_type is replaced by type in data object
(data[i].trial_type => data[i].type)  */
  module.getTrialsOfType = function(trial_type) {
    ...
    for (var i = 0; i < data.length; i++) {
      if (data[i].type == trial_type) {  // previous: (data[i].trial_type == trial_type)
        trials.push(data[i]);
      }
    }
